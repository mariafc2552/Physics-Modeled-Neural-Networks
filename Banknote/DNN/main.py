...
def _evaluate_epoch(
    ...
    *,
    ...
    """
    Evaluate one epoch for Banknote Authentication binary classification.
...
def fit(
    ...
    *,
    ...
    """
    Train, validate and test a DNN model for Banknote Authentication binary
    classification.
...
if __name__ == "__main__":
    ...
    baserundir = os.path.join(rootdir, "runs_dnn_banknote")
    ...
    if input_size != 4:
        raise ValueError(
            f"Banknote Authentication requires input_size=4, "
            f"but input_size={input_size} was provided in config.yaml."
        )

    if output_size != 1:
        raise ValueError(
            f"Banknote Authentication binary classification with BCEWithLogitsLoss "
            f"requires output_size=1, but output_size={output_size} was provided "
            f"in config.yaml."
        )

    if "dropout" in config.get("model", {}):
        print(
            "Warning: dropout was found in config.yaml but will be ignored "
            "because this DNNBlock does not define dropout."
        )

    if "use_batch_norm" in config.get("model", {}):
        print(
            "Warning: use_batch_norm was found in config.yaml but will be ignored "
            "because this DNNBlock does not define batch normalization."
        )
