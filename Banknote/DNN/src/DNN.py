...
class DNNBlock(nn.Module):
    """
    Fully connected feed-forward neural network for binary classification.

    This version is adapted for the Banknote Authentication dataset.
    ...
    The model returns a raw logit. Do not apply sigmoid inside the model.
    Use BCEWithLogitsLoss in the training script.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if output_size != 1:
            raise ValueError(
                "output_size must be 1 for Banknote Authentication binary "
