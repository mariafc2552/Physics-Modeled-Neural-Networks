...
def compute_metrics(
    ...
    """
    Compute binary classification metrics for Banknote Authentication.
...
if __name__ == "__main__":
    ...
    if X_train.shape[1] != 4:
        print(
            f"Warning: Banknote Authentication usually has 4 input features, "
            f"but X_train has {X_train.shape[1]} features."
        )

    train_labels = sorted(np.unique(y_train).tolist())
    valid_labels = sorted(np.unique(y_valid).tolist())
    ...
    if len(np.unique(y_train)) != 2:
        raise ValueError(
            "This script expects binary classification labels with two classes."
        )

    if not set(train_labels).issubset({0, 1}):
        raise ValueError(
            f"Banknote labels must be encoded as 0 and 1. "
            f"Found train labels: {train_labels}"
        )
    ...
    if best_model is not None:
        ...
        try:
            best_tree_text = export_text(best_model)
