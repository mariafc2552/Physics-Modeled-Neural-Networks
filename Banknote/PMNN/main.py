...
def fit(
    ...
    *,
    ...
    """
    Train, validate and test a FitzHugh--Nagumo PMNN for Banknote Authentication
...
if __name__ == "__main__":
    ...
    with open(hyperparameter_results_filename, "w", newline="") as result_file:
    ...
    if input_size != 4:
        print(
            f"Warning: Banknote Authentication usually has input_size=4, "
            f"but input_size={input_size} was provided."
        )

    if output_size != 2:
        print(
            f"Warning: Banknote Authentication usually has output_size=2, "
        ...
        for t_end in t_end_values:
            for batch_size in batch_size_values:
                for lr in lr_values:
                    ...
                    config_name = (
                        f"t_step_{tag_value(t_step)}"
                        f"_t_end_{tag_value(t_end)}"
                    ...
                    with open(hyperparameter_results_filename, "a", newline="") as result_file:
                        writer = csv.DictWriter(result_file, fieldnames=header)
                        writer.writerow(
                            {
                                "file_name": f"{config_name}.txt",
