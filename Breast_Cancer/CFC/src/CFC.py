...
class CFCblock(nn.Module):
    """
    Closed-form Continuous-time Neural Network for Breast Cancer Wisconsin
    ...
    artificial time step and 30 input features.

    Architecture:

        input vector of 30 features
            -> unsqueeze temporal dimension
            -> CfC(input_size=30, units=num_units)
            -> last hidden representation
            -> Linear(num_units, output_size)

    For Breast Cancer classification with CrossEntropyLoss:
        ...
        *,
        ...
        Args:
            ...
            * output_size: number of output classes.
              For Breast Cancer with CrossEntropyLoss, use 2.
    ...
    def _prepare_sequence(self, x: torch.Tensor) -> torch.Tensor:
        """
        Prepare Breast Cancer tabular data for CfC.

        Accepted inputs:

            1. x.shape = (batch_size, 30)
               This is converted to (batch_size, 1, 30).

            2. x.shape = (batch_size, 1, 30)
               This is already a valid CfC input.

        Returns:
            Tensor with shape (batch_size, 1, 30).
        """
        if x.dim() == 1:
            x = x.view(1, -1)
