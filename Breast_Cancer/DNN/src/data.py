...
class Reader:
    """
    Class for reading Breast Cancer Wisconsin Diagnostic dataset splits from directory.

    Expected structure:

        datadir/
        ├── train/
        │   ├── breast_cancer__b001.npz
        │   ├── breast_cancer__b002.npz
        │   └── ...
        ├── valid/
        │   └── ...
        └── test/
    ...
    def read(f):
        ...
        Outputs:
            * features: torch.FloatTensor, shape (batch_size, 30)
...
def save_split(split_name, Xs, ys, datadir, batch_size, base_name):
    ...
    incomplete batch. This is important for small datasets such as Breast
    Cancer Wisconsin, where the test split may contain fewer samples than
...
if __name__ == "__main__":
    ...
    csv_path = os.path.join(srcdir, "breast_cancer.csv")
    ...
    metadata = {
        ...
        "label_mapping": {
            str(dataset.target_names[0]): 0,
            str(dataset.target_names[1]): 1,
        },
        "split_ratios": {
