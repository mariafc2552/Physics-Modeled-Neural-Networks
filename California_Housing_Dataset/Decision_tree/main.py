...
def write_metrics_report(
    ...
    *,
    ...
    with open(file_path, "w") as f:
        ...
        f.write("Dataset:\n")
        f.write("California Housing\n")
        f.write("Task: regression\n")
        f.write("Target: median_house_value\n\n")
...
if __name__ == "__main__":
    ...
    results_base = config["paths"].get(
        "results",
        "runs_decision_tree_california_housing",
    )
    ...
    if X_train.shape[1] != 8:
        print(
            f"Warning: California Housing normally has 8 features, "
    ...
    if not np.issubdtype(y_train.dtype, np.number):
        raise ValueError("This script expects a numerical continuous target.")
    ...
    for idx, params in enumerate(param_grid, start=1):
        ...
        try:
            feature_names = CALIFORNIA_FEATURE_NAMES
    ...
    if best_model is not None:
        ...
        best_model_filename = config["paths"].get(
            "model",
            "decision_tree_california_housing.pkl",
        )
