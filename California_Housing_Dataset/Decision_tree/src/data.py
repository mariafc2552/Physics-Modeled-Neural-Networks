...
class Reader:
    """
    Class for reading Diabetes dataset splits from directory.
    ...
    Each .npz contains:
        * features: array of shape (batch_size, 10)
        * target:   array of shape (batch_size, 1)

    For this dataset:
        * n_features = 10
        * target is continuous

    This is a regression problem, so the target is compatible with MSELoss.
    """
    ...
    def read(f):
        ...
        Outputs:
            * features: torch.FloatTensor, shape (batch_size, 10)
            * target: torch.FloatTensor, shape (batch_size, 1)
        """
        data = dict(np.load(f))
...
def save_split(split_name, Xs, ys, datadir, batch_size, base_name):
    ...
    This is important for small datasets such as Diabetes.
    """
    ...
    if batch_size is None:
        ...
        np.savez(
            ...
            target=ys.numpy().astype(np.float32).reshape(-1, 1),
        )
...
if __name__ == "__main__":
    ...
    #   * target is a continuous value representing disease progression
    # -------------------------------------------------------------------------

    dataset = load_diabetes()

    X_np = dataset.data.astype(np.float32)
    y_np = dataset.target.astype(np.float32)

    df = pd.DataFrame(X_np, columns=dataset.feature_names)
    df["target"] = y_np

    csv_path = os.path.join(srcdir, "diabetes.csv")
