...
def fit(model,
    ...
    save_loss_curves(train_losses, valid_losses, t_step, t_end, batch_size, lr, results_folder)
...
if __name__ == "__main__":
    ...
    baserundir = os.path.join(rootdir, "runs_pmnn")
    ...
    t_step_values: Sequence[int] = config["model"]["t_step"]
    t_end_values: Sequence[int] = config["model"]["t_end"]
            ...
            for batch_size in batch_size_values:
                for lr in lr_values:
                    ...
                    if t_end % t_step != 0 or t_step > t_end:
                        print(
                            f"Invalid configuration: t_end ({t_end}) is not divisible by t_step ({t_step}) "
                            f"or t_step is greater than t_end. Ignoring configuration..."
                        )
                        continue

                    print(f"Evaluating t_step = {t_step}, t_end = {t_end}, batch_size = {batch_size}, lr = {lr}")

                    model = PMNNBlock(input_size=8, hidden_size=2, output_size=1)
                    ...
                    # === cambio mínimo: crear carpeta por configuración y usarla en todos los artefactos ===
                    config_results_folder = os.path.join(
                        rundir,
                        f"t_step_{t_step}_t_end_{t_end}_batch_size_{batch_size}_lr_{lr}"
                    )
                    ...
                    val_loss, train_losses, val_losses, test_loss, elapsed_str, epoch_times = fit(
                        ...
                        results_folder=config_results_folder,  # <-- usar carpeta de la configuración
                        checkpoint_path=os.path.join(
                            config_results_folder,  # <-- guardar checkpoint en la carpeta de la configuración
                    ...
                    log_filename = os.path.join(
                        ...
                        f"t_step_{t_step}_t_end_{t_end}_batch_size_{batch_size}_lr_{lr}.txt"
                    )
                    with open(log_filename, 'w') as log_file:
                        log_file.write(
                            f"Evaluating parameters: t_step = {t_step}, t_end = {t_end}, batch_size = {batch_size}, lr = {lr}\n"
                        )
                    ...
                    with open(hyperparameter_results_filename, 'a') as result_file:
                        result_file.write(
                            f"t_step_{t_step}_t_end_{t_end}_batch_size_{batch_size}_lr_{lr}.txt, {test_loss}\n"
                        )

                    if val_loss < best_val_loss:
                        best_val_loss = val_loss
                        best_test_loss = test_loss
                        best_params = (t_step, t_end, batch_size, lr)

    if best_params is not None:
        print(f"\nBest parameters found:")
        print(f"t_step: {best_params[0]}")
        print(f"t_end: {best_params[1]}")
        print(f"batch_size: {best_params[2]}")
        ...
        best_config_foldername = f"t_step_{best_params[0]}_t_end_{best_params[1]}_batch_size_{best_params[2]}_lr_{best_params[3]}"
        ...
        with open(os.path.join(dst_best_folder, "best_summary.txt"), "w") as fsum:
            ...
            fsum.write(f"t_step: {best_params[0]}\n")
            fsum.write(f"t_end: {best_params[1]}\n")
            fsum.write(f"batch_size: {best_params[2]}\n")
            fsum.write(f"learning rate: {best_params[3]}\n")
