...
def _unwrap_grid_value(value: Any) -> Any:
    ...
    This is useful when some parameters are stored as one-element lists
...
class CFCblock(nn.Module):
    """
    Closed-form Continuous-time Neural Network for the Diabetes regression dataset.
    ...
    Data interpretation:
        ...
        CfC input:
            x.shape = (batch_size, 1, 10)

    Therefore, each patient/sample is interpreted as a sequence with one
    artificial time step and 10 input features.

    Architecture:

        input vector of 10 features
            -> unsqueeze temporal dimension
            -> CfC(input_size=10, units=num_units)
    ...
    The model returns a continuous scalar prediction. Do not apply sigmoid
    or softmax inside this model.
    """
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            * batch_first: whether input shape is (batch, sequence, features).
    ...
    def _prepare_sequence(self, x: torch.Tensor) -> torch.Tensor:
        """
        Prepare Diabetes tabular data for CfC.

        Accepted inputs:

            1. x.shape = (batch_size, 10)
               This is converted to (batch_size, 1, 10).

            2. x.shape = (batch_size, 1, 10)
               This is already a valid CfC input.

        Returns:
            Tensor with shape (batch_size, 1, 10).
        """
        if x.dim() == 1:
            x = x.view(1, -1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            * y_hat: tensor with shape (batch_size, 1).
        """
