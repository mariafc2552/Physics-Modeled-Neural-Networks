...
if __name__ == "__main__":
    ...
    baserundir = os.path.join(rootdir, "runs_dnn_diabetes")
    ...
    input_size = int(config["model"].get("input_size", 10))
    output_size = int(config["model"].get("output_size", 1))

    if input_size != 10:
        raise ValueError(
            f"Diabetes requires input_size=10, "
            f"but input_size={input_size} was provided in config.yaml."
        )

    if output_size != 1:
        raise ValueError(
            f"Diabetes requires output_size=1 for regression, "
