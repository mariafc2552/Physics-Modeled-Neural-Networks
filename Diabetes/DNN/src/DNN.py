...
class DNNBlock(nn.Module):
    """
    Fully connected feed-forward neural network for regression.

    This version is adapted for the Diabetes dataset.
    ...
    softmax or argmax. Use MSELoss in the training script.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if input_size != 10:
            raise ValueError(
                "input_size must be 10 for the Diabetes dataset."
            )
        ...
        if output_size != 1:
            raise ValueError(
                "output_size must be 1 for Diabetes regression "
                "with MSELoss."
            )
