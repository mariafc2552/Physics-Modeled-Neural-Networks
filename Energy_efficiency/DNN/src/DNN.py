...
class DNNBlock(nn.Module):
    """
    Fully connected feed-forward neural network for multi-output regression.
    ...
    def __init__(
        ...
        *,
        ...
        if input_size != 8:
            raise ValueError(
                "input_size must be 8 for the Energy Efficiency dataset."
            )
        ...
        if output_size != 2:
            raise ValueError(
                "output_size must be 2 for Energy Efficiency multi-output regression "
                "with MSELoss."
            )
