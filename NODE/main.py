...
if __name__ == "__main__":
    ...
    baserundir = os.path.join(rootdir, "runs_node")
        ...
        for hidden_dimension in hidden_dims:
            for batch_size in batch_size_values:
                for lr in lr_values:
                    ...
                    print(f"Evaluating batch_size = {batch_size}, lr = {lr}, num_layer = {layer}, hidden_dim = {hidden_dimension}")

                    model = NODEblock(input_size=8, output_size=1, hidden_dim=hidden_dimension, num_layers=layer)
                    ...
                    with open(log_filename, 'w') as log_file:
                        log_file.write(
                            f"Evaluating parameters: batch_size = {batch_size}, lr = {lr}, num_layer = {layer}, hidden_dim = {hidden_dimension}\n"
                        )
                    ...
                    with open(hyperparameter_results_filename, 'a') as result_file:
                        result_file.write(
                            f"batch_size_{batch_size}_lr_{lr}_num_layer_{layer}_hidden_dim_{hidden_dimension}.txt, {test_loss}\n"
                        )

                    if val_loss < best_val_loss:
                        best_val_loss = val_loss
                        best_test_loss = test_loss
                        best_params = (batch_size, lr, layer, hidden_dimension)
    ...
    if best_params is not None:
        ...
        print(f"hidden size: {best_params[3]}")
        print(f"Best validation_loss: {best_val_loss}")
        print(f"Test loss for this config: {best_test_loss}")

        best_config_foldername = f"batch_size_{best_params[0]}_lr_{best_params[1]}_num_layer_{best_params[2]}_hidden_size_{best_params[3]}"
        ...
        with open(os.path.join(dst_best_folder, "best_summary.txt"), "w") as fsum:
            ...
            fsum.write(f"hidden size: {best_params[3]}\n")
